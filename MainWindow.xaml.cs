...
namespace CyberBot_POE
{
    public partial class MainWindow : Window
    {
        ...
        private void ProcessMessage(string message)
        {
            ...
            if (_currentQuestionIndex < _quizQuestions.Count && step != "description" && step != "reminder")
            {
                string quizResponse = CheckQuizAnswer(message);
                if (!string.IsNullOrEmpty(quizResponse))
                {
                    Bot(quizResponse);
                    return;
                }
        ...
        private string CheckQuizAnswer(string message)
        {
            string answer = message.ToUpper().Trim();

            if (_currentQuestionIndex >= _quizQuestions.Count)
                return null;

            var question = _quizQuestions[_currentQuestionIndex];
            int selectedAnswer = -1;

            // Check if user typed A, B, C, or D
            ...
            else return null; // Not a quiz answer
            ...
            {
                Bot($"❌ Incorrect! The correct answer was: {question.Options[question.CorrectAnswer]}\n\n{question.Explanation}");
            }
            ...
            {
                Bot($"🎉 Quiz Completed!\n\nYour final score: {_score} out of {_quizQuestions.Count}");
            }

            return ""; // Already sent response via Bot()
        }
        ...
        private void ShowQuizQuestion()
        {
            ...
            {
                var question = _quizQuestions[_currentQuestionIndex];
                string quizText = $"🎯 Question {_currentQuestionIndex + 1} of {_quizQuestions.Count}:\n\n";
                quizText += $"{question.QuestionText}\n\n";
                quizText += $"A) {question.Options[0]}\n";
                quizText += $"B) {question.Options[1]}\n";
                quizText += $"C) {question.Options[2]}\n";
                quizText += $"D) {question.Options[3]}\n\n";
                quizText += "Type your answer (A, B, C, or D)";
        ...
        private string GetSecurityResponse(string message)
        {
            ...
            Dictionary<string, string> responses = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "cyber crime", "Cyber crime includes illegal activities like hacking, scams, identity theft, and spreading malware." }
            };
